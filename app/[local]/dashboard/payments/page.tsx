      ...
      <main className="flex-1 p-6 md:p-8 space-y-8  w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-stone-100 shadow-sm p-6 overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="font-bold text-stone-900">
                  {t.PaymentCharts.revenueVsTime}
                </h3>
                <p className="text-xs text-stone-500 mt-1">
                  {t.PaymentCharts.monthlyRevenueTrend}
                </p>
              </div>
            ...
            <div className="h-64 flex items-end justify-between gap-2 px-2">
              {[40, 70, 45, 90, 65, 80, 55, 95, 75, 40, 85, 60].map(
                (h, idx) => (
                  <div
                    key={idx}
          ...
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
            <h3 className="font-bold text-stone-900 mb-6">
              {t.PayoutSchedule.title}
            </h3>

            <div className="space-y-6">
              <div className="p-4 bg-orange-50 rounded-2xl border border-orange-100">
                <p className="text-xs font-semibold text-orange-600 uppercase mb-1">
                  {t.PayoutSchedule.nextDeposit}
                </p>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-stone-500">
                    {t.PayoutSchedule.amountToTransfer}
                  </span>
                ...
                <div className="flex justify-between items-center text-sm">
                  <span className="text-stone-500">
                    {t.PayoutSchedule.verificationProgress}
                  </span>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-stone-100 flex items-center justify-between">
            <div>
              <h3 className="font-bold text-stone-900">
                {t.TransactionTable.recentTransactions}
              </h3>
              <p className="text-xs text-stone-500 mt-1">
                {t.TransactionTable.liveFeed}
              </p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left rtl:text-right">
              <thead className="bg-stone-50/50 text-stone-500 text-xs uppercase font-semibold">
                <tr>
                  <th className="px-6 py-4">{t.TransactionTable.invoiceId}</th>
                  <th className="px-6 py-4">{t.TransactionTable.customer}</th>
                  <th className="px-6 py-4">{t.TransactionTable.status}</th>
                  <th className="px-6 py-4">{t.TransactionTable.method}</th>
                  <th className="px-6 py-4">{t.TransactionTable.amount}</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        ...
                        />
                        {tx.status === "success"
                          ? t.TransactionTable.success
                          : tx.status === "pending"
                            ? t.TransactionTable.pending
